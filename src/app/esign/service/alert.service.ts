...
export class AlertService {
  ...
  showDialog(title: string, message: string, type?: DialogType, okCallback?: (val?: any) => any,
    ...
    this.dialogs.next({
      title, message: message, type: type, okCallback: okCallback,
      cancelCallback: cancelCallback, okLabel: okLabel, cancelLabel: cancelLabel,
  ...
  showStickyMessage(data: string | string[], separatorOrDetail?: string,
    severity?: MessageSeverity, error?: any, onRemove?: () => any) {
    ...
    if (data instanceof Array) {
      ...
      if (error) {
        const msg = `Severity: "${MessageSeverity[severity]}", Summary: "${data}",
        Detail: "${separatorOrDetail}", Error: "${error}"`;
        switch (severity) {
  ...
  private showMessageHelper(summary: string, detail: string, severity: MessageSeverity, isSticky: boolean, onRemove?: () => any) {
    console.log('show message helper');
    console.log(summary);
  ...
  showValidationError(errorCaption: string, errorMessage: string) {
    this.resetStickyMessage();
    this.showStickyMessage(errorCaption,
      errorMessage, MessageSeverity.error);
  }
